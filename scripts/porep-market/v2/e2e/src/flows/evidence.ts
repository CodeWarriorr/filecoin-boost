...
export async function submitEvidenceBatchAndAssertClaimCoverage(
  ...
  const txHash = await evm.send(context.config.addresses.poRepMarket, "submitEvidenceBatch(uint256,bytes)", [
    accepted.dealId,
    evidenceData
  ]);
...
export async function activateEvidenceAndAssertDealActive(
  ...
  const txHash = await evm.send(context.config.addresses.poRepMarket, "activateEvidence(uint256,bytes)", [accepted.dealId, "0x"]);
...
export async function assertActivationBeforeEvidenceStaysAccepted(
  ...
  await evm.send(context.config.addresses.poRepMarket, "activateEvidence(uint256,bytes)", [accepted.dealId, "0x"]);
...
export async function expectDoubleActivationToFail(
  ...
  const error = await expectCustomError(
    () => evm.simulate(context.config.addresses.poRepMarket, "activateEvidence(uint256,bytes)", [active.dealId, "0x"]),
