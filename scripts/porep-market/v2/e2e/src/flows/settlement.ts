...
export async function configureSettlementCadenceForDevnet(
  ...
  const txHash = await evm.send(context.config.addresses.poRepMarket, "setMinEpochsBetweenSettlements(uint256,uint256)", [
    accepted.dealId,
    minEpochs
  ]);
...
export async function refreshEvidenceStatusAndAssertActive(
  ...
  const txHash = await evm.send(context.config.addresses.poRepMarket, "refreshEvidenceStatus(uint256,bytes)", [
    active.dealId,
    evidenceData
  ]);
