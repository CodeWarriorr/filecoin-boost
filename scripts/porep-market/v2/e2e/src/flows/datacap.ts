...
import { ensureBoostReady, importPieceIntoBoost } from "../devnet/boost.js";
...
export async function importPieceAndWaitForProviderClaim(
  ...
  ensureBoostReady(context);
  const importResult = await importPieceIntoBoost(context, {
...
function computeDataCapBatchCalldata(
  ...
  const output = runRequired(
    "forge",
    ["script", join(context.scriptsRoot, "v2/mocks/ComputeDataCapBatchCalldata.s.sol"), "--rpc-url", context.config.rpcUrl],
