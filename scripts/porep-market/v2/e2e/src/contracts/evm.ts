...
export class Evm {
  ...
  async sendWithPrivateKey(
    ...
    const result = run("cast", castArgs, this.context.boostRoot);
  ...
  receipt(txHash: string): TxReceipt {
    const output = runRequired("cast", ["receipt", "--rpc-url", this.context.config.rpcUrl, txHash, "--json"], this.context.boostRoot);
  ...
  blockNumber(): bigint {
    return BigInt(runRequired("cast", ["block-number", "--rpc-url", this.context.config.rpcUrl], this.context.boostRoot).split(/\s+/)[0] ?? "0");
  }
  ...
  abiEncode(types: string, value: string | number | bigint): string {
    return runRequired("cast", ["abi-encode", types, String(value)], this.context.boostRoot);
  }

  calldata(signature: string, args: Array<string | number | bigint | boolean>): string {
    return runRequired("cast", ["calldata", signature, ...args.map(String)], this.context.boostRoot);
  }
}
