...
> Current script routing: the generic flow below describes the original V1
> `Client` allocation model. New local testing should start with
> `scripts/porep-market/README.md`, `just porep-v1-*` for maintenance branch
> `v1`, and `just porep-v2-*` for current branch `main`. The current V2 full
> happy path is blocked at dummy `DataCapEvidenceAdapter` activation/status
> methods; `just porep-v2-proposal-smoke` and
> `just porep-v2-validator-rail-smoke` cover the implemented boundary.
