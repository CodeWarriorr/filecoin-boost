...
#   CLAIM_MAX_ATTEMPTS   - max polling attempts (default: 900)
#   CLAIM_POLL_SECONDS   - seconds between polls (default: 1)
# State in: ALLOC_ID
set -euo pipefail
...
publish_batches() {
    local verbose="${1:-0}"
    if [ "$verbose" = "1" ]; then
        docker exec lotus-miner lotus-miner sectors batching precommit --publish-now 2>&1 || true
    ...
    if [ -n "$TARGET_START_EPOCH" ]; then
        ...
        if [ "$HEAD_EPOCH" -ge $((TARGET_START_EPOCH - 3)) ] && [ "$HEAD_EPOCH" -le $((TARGET_START_EPOCH + 3)) ]; then
            ...
            for _ in $(seq 1 20); do
                HEAD_EPOCH=$(docker exec lotus lotus chain head --height 2>/dev/null | tr -d '\r\n ' || echo 0)
                [ "$HEAD_EPOCH" -eq "$TARGET_START_EPOCH" ] || break
                echo "  Publishing commit at target epoch $HEAD_EPOCH"
                docker exec lotus-miner lotus-miner sectors batching commit --publish-now 2>&1 || true
                if claim_exists; then
                    CLAIM_FOUND=1
                    echo "  Claim $ALLOC_ID confirmed on-chain"
